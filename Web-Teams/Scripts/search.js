...
function createSearchOutput(listOfAyHaga) {
  for (i = 0; i < listOfAyHaga.length; i++) {
    ...
    var firstElementParent = document.getElementById(
      "containCreatedElementsSearch"
    );
    ...
    createFirstParent.setAttribute(
      "data-Description",
      listOfAyHaga[i].description
    );
    ...
    createFirstParent.addEventListener("click", function (event) {
      var popUpGrandContainer = document.getElementsByClassName(
        "searchPopUpGrandContainer"
      )[0];
      ...
      //pop up img
      var popUpImg =
        popUpGrandContainer.getElementsByClassName("searchPopUpImg")[0];
      ...
      //pop up title
      var popUpTitle =
        popUpGrandContainer.getElementsByClassName("searchPopUpTitle")[0];
      popUpTitle.innerHTML = elememtPop.getAttribute("data-Title");

      //pop up description
      var popUpDescription = popUpGrandContainer.getElementsByClassName(
        "searchPopUpDescription"
      )[0];
      popUpDescription.innerHTML = elememtPop.getAttribute("data-Description");

      //pop up price
      var popUpPrice =
        popUpGrandContainer.getElementsByClassName("searchPopUpPrice")[0];
      ...
      addToCartBtn.setAttribute(
        "data-Img",
        elememtPop.getAttribute("data-Img")
      );
      addToCartBtn.setAttribute(
        "data-Title",
        elememtPop.getAttribute("data-Title")
      );
      addToCartBtn.setAttribute(
        "data-Description",
        elememtPop.getAttribute("data-Description")
      );
      addToCartBtn.setAttribute(
        "data-Price",
        elememtPop.getAttribute("data-Price")
      );
    });
...
function closePopUp() {
  document
    .getElementsByClassName("searchPopUpGrandContainer")[0]
    .classList.add("visibleSearch");
}
...
function searchInputWords(txt) {
  ...
  for (i = 0; i < listOfSearch.length; i++) {
    ...
    if (selectValue == "All") {
      ...
      if (
        searchLoopTitle.includes(txt.toLowerCase()) &&
        listOfSearchOfI.category == selectValue
      ) {
        arrayOfSelectedSearch.push(listOfSearchOfI);
      }
    }
