...
function activeDeactive(elem) {
  if (elem) {
    document
      .getElementsByClassName("menuButton-active")[0]
      .classList.remove("menuButton-active");
...
function buildPageWithData(urlRequest) {
  ...
  categoryTypeReq.onreadystatechange = function () {
    if (categoryTypeReq.readyState === 4 && categoryTypeReq.status == 200) {
      ...
      listOfCategoryTypeItems = JSON.parse(categoryTypeReq.responseText)[
        "record"
      ];
...
function nextTo() {
  ...
  for (var i = 0; i < 4; i++) {
    ...
    if (counter > listOfCategoryTypeItems.length - 1) {
      document
        .querySelectorAll(".menuItemsDetails")
      ...
      document
        ...
        [i].setAttribute(
          "data-idOfItem",
          listOfCategoryTypeItems[counter]["id"]
        );

      document.querySelectorAll(".menuItemsDetails h2")[i].innerText =
        listOfCategoryTypeItems[counter]["title"];

      document.querySelectorAll(".description")[i].innerText =
        listOfCategoryTypeItems[counter]["description"];

      document.querySelectorAll(".menuItemsDetails p")[i].innerText =
        listOfCategoryTypeItems[counter]["price"] + " $";

      document.querySelectorAll(".menuItemsDetails img")[i].src =
        listOfCategoryTypeItems[counter]["image"];
      counter++;
    }
  }
